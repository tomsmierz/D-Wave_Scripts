...
output_file = "ising_instance_diag_5x5.txt"  # Change the output file name if needed
    ...
    if m > 1:
        for i in range(m - 1):
            for j in range(m - 1):
                node = i * m + j
                lower_left = node * 4 + 5
                upper_right = (node + m) * 4 + 1

                for spin1 in range(lower_left, lower_left + 4):
                    for spin2 in range(upper_right, upper_right + 4):
                        if is_not_pair_in_ising_data(spin1, spin2, ising_data):
                            Jij = random.uniform(-1, 1)
                            ising_data.append((spin1, spin2, Jij))
