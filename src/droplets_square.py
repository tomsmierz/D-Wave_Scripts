...
def create_independent_set_of_union_states(json_directory, h5_directory, best_found_path, instance_path, history_directory, metric: Optional[str], **kwargs):
    min_df = pd.read_csv(best_found_path, index_col=0, delimiter=',', quotechar='"')
    min_df.index = min_df.index.map(lambda x: str(x).zfill(3))
    ...
    spinglass_states1 = read_all_json_files(json_directory, beta1, min_df)
    spinglass_states2 = read_all_json_files(json_directory, beta2, min_df)
    spinglass_states3 = read_all_json_files(json_directory, beta3, min_df)
    spinglass_states4 = read_all_json_files(json_directory, beta4, min_df)
    spinglass_states5 = read_all_json_files(json_directory, beta5, min_df)
    spinglass_states6 = read_all_json_files(json_directory, beta6, min_df)
...
if __name__ == '__main__':
    result = create_independent_set_of_union_states(json_directory, h5_directory, minimum_path, instance_path, output_directory_union, 
                                                    "connected", beta1=BETA1, beta2=BETA2, beta3=BETA3, beta4=BETA4, beta5=BETA5, beta6=BETA6, inst=INST)
