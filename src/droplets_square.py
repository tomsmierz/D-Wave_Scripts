...
from droplets import (xor, hamming_dist, create_spin_glass_peps_graph, connected_hamming_dist,
    ...
    find_max_set)
from read_data import (read_all_json_files, read_all_csv_files, read_json_files, read_h5_files, get_state_energy_from_dwave)
...
json_directory = os.path.join(root, "droplets", "square_diagonal", "square_diagonal", "50x50_ground_droplets_betas")
h5_directory = os.path.join(root, "energies", "sbm", "square_diagonal", "square_diag_50x50", "filtered_files_hd200_i020_ar005")
cplex_directory = os.path.join(root, "energies", "cplex", "square_50_results.csv")
minimum_path = os.path.join(root, "droplets", "square_diagonal", "results", "50x50", "squarediag50_all.csv")
instance_path = os.path.join(root, "instances", "square_diag_instances", "square_diag_50x50", "single")
output_directory_union = os.path.join(root, "droplets", "square_diagonal", "results", "50x50", "union_ar005_R025_beta", f"{INST}")
output_directory_solver = os.path.join(root, "droplets", "square_diagonal", "results","50x50", "solver_ar005_R025_beta", f"{INST}")

# json_directory = os.path.join(root, "droplets", "square", "50x50_ground_droplets_betas")
...
# output_directory_union = os.path.join(root, "droplets", "square", "results", "50x50", "union_ar005_R025_beta", f"{INST}")
# output_directory_solver = os.path.join(root, "droplets", "square", "results","50x50", "solver_ar005_R025_beta", f"{INST}")
...
def create_independent_set_of_union_states(json_directory, h5_directory, best_found_path, instance_path, history_directory, metric: Optional[str], is_Z2: bool = False, **kwargs):
    min_df = pd.read_csv(best_found_path, index_col=0, delimiter=',', quotechar='"')
    min_df.index = min_df.index.map(lambda x: str(x).zfill(3))
    ...
    spinglass_states1 = read_all_json_files(json_directory, beta1, APPROX_RATIO, min_df, is_Z2)
    spinglass_states2 = read_all_json_files(json_directory, beta2, APPROX_RATIO, min_df, is_Z2)
    spinglass_states3 = read_all_json_files(json_directory, beta3, APPROX_RATIO, min_df, is_Z2)
    spinglass_states4 = read_all_json_files(json_directory, beta4, APPROX_RATIO, min_df, is_Z2)
    spinglass_states5 = read_all_json_files(json_directory, beta5, APPROX_RATIO, min_df, is_Z2)
    spinglass_states6 = read_all_json_files(json_directory, beta6, APPROX_RATIO, min_df, is_Z2)
    ...
    for filename in tqdm(os.listdir(instance_path)):
        ...
        if os.path.isfile(file):
            ...
            state_energy_tn4 = spinglass_states4[name]
            state_energy_tn5 = spinglass_states5[name]
            state_energy_tn6 = spinglass_states6[name]

            state_energy_sbm = sbm_states[name]
            union_data = {}
            for nt in [state_energy_tn1, state_energy_tn2, state_energy_tn3, state_energy_tn4, state_energy_tn5, state_energy_tn6, state_energy_sbm]:
...
if __name__ == '__main__':
    result = create_independent_set_of_union_states(json_directory, h5_directory, minimum_path, instance_path, output_directory_union, 
                                                    "connected", is_Z2, beta1=BETA1, beta2=BETA2, beta3=BETA3, beta4=BETA4, beta5=BETA5, beta6=BETA6, inst=INST)
    ...
    name, count = find_droplets_in_union(output_directory_union, json_directory, cplex_directory, h5_directory, output_directory_solver, minimum_path, instance_path, "SpinGlass", is_Z2, 
                                    beta=BETA1, eng=ENG, bd=BD, ms=MAX_STATES, inst=INST)
    name, count = find_droplets_in_union(output_directory_union, json_directory, cplex_directory, h5_directory, output_directory_solver, minimum_path, instance_path, "SpinGlass", is_Z2, 
                                    beta=BETA2, eng=ENG, bd=BD, ms=MAX_STATES, inst=INST)
    name, count = find_droplets_in_union(output_directory_union, json_directory, cplex_directory, h5_directory, output_directory_solver, minimum_path, instance_path, "SpinGlass", is_Z2, 
                                    beta=BETA3, eng=ENG, bd=BD, ms=MAX_STATES, inst=INST)
    name, count = find_droplets_in_union(output_directory_union, json_directory, cplex_directory, h5_directory, output_directory_solver, minimum_path, instance_path, "SpinGlass", is_Z2, 
                                    beta=BETA4, eng=ENG, bd=BD, ms=MAX_STATES, inst=INST)
    name, count = find_droplets_in_union(output_directory_union, json_directory, cplex_directory, h5_directory, output_directory_solver, minimum_path, instance_path, "SpinGlass", is_Z2, 
                                    beta=BETA5, eng=ENG, bd=BD, ms=MAX_STATES, inst=INST)
    name, count = find_droplets_in_union(output_directory_union, json_directory, cplex_directory, h5_directory, output_directory_solver, minimum_path, instance_path, "SpinGlass", is_Z2, 
                                    beta=BETA6, eng=ENG, bd=BD, ms=MAX_STATES, inst=INST)
