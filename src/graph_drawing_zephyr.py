...
if __name__ == '__main__':
    df = pd.read_csv(os.path.join(root, "instances", "zephyr_random", "Z3", "RAU", "001_sg.txt"), header=None, sep=" ",
                     names=["n1", "n2", "v"])
    ...
    # labels = {node: dnx.zephyr_coordinates(3).nice_to_linear(node) for node in z3.nodes()}
    sm = plt.cm.ScalarMappable(cmap=plt.cm.seismic, norm=plt.Normalize(vmin=-1, vmax=1))
    sm.set_array([])
    # dnx.draw_zephyr(z3, node_color='black', node_size=30, edge_color=J.values(), edge_cmap=plt.cm.seismic,
    #                  edge_vmin=-1, edge_vmax=1, ax=ax_graph)
    dnx.draw_zephyr(z3, node_color='black', node_size=30)
