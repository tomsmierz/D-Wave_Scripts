...
for topology in ["zephyr"]:  # ["pegasus", "zephyr"]:
    ...
    sampler = (
        DWaveSampler(solver="Advantage_system6.1")
        if topology == "pegasus"
        else DWaveSampler(solver="Advantage2_prototype1.1")
    )
    symbol = "P" if topology == "pegasus" else "Z"
    for j in sizes:
        size = j
        for i in tqdm(range(1, 101)):
            ...
            for category in ["AC3"]:  # ["AC3", "RCO", "RAU", "CBFM-P"]:
                ...
                for annealing_time in [2000]:  # [2, 200, 2000]:
                    num_reads = (
                        445
                        if annealing_time == 2000
                        else 2500
                        if annealing_time == 200
                        else 6000
                    )
                    ...
                    df.to_csv(
                        f"../energies/{topology}_random/{symbol}{size}/{category}/{instance_number}_{annealing_time}_{num_reads}.csv"
                    )
